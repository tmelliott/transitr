...
List construct_network (List nw, List shapes)
{
			...
			if (result_n.size () == 0) 
			{
				...
				for (auto it = edge_tree.qbegin (
						bgi::satisfies ([&](evalue_t const& v) 
						{
							...
							if (std::find (removed_edges.begin (), 
								removed_edges.end (), v.second) != removed_edges.end ()) 
								return false;
			    ...
			    if (result_e.size ()) 
			    {
					...
					auto res = std::min_element (result_e.begin (), result_e.end (),
						[] (std::tuple<evalue_t, node_t, double> s1, 
							std::tuple<evalue_t, node_t, double> s2) 
						{
	...
	for (auto it = edge_tree.qbegin ( bgi::satisfies ([&](evalue_t const& v) { return true; })); 
		 it != edge_tree.qend (); ++it)
    {
      	if (std::find (removed_edges.begin (), removed_edges.end (), it->second) != removed_edges.end ()) 
      		continue;
